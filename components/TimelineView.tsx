...
import { Search, Clock, User, Copy, Check, MessageSquare } from 'lucide-react'
...
export function TimelineView({ timeline }: { timeline: TimelineSegment[] }) {
  ...
  return (
    <div className="space-y-4">
      {/* Controls Bar */}
      <div className="flex flex-col sm:flex-row gap-3 items-center justify-between pb-2 border-b border-zinc-800/80">
        {/* Search Input */}
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full bg-zinc-900 border border-zinc-800 rounded-xl pl-8 pr-3 py-1.5 text-xs text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-blue-500 transition-colors"
          />
        </div>
      ...
      <div className="space-y-3">
        {filtered.map((seg, i) => {
          ...
          return (
            <div
              key={i}
              className="flex items-start gap-3.5 p-4 rounded-2xl bg-zinc-950/80 border border-zinc-800/80 hover:border-zinc-700/80 transition-all group"
            >
              {/* Speaker Avatar */}
              <div
